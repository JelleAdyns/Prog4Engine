...
namespace dae
{
	...
	public:
		...
		void AddComponent(Args&&... args)
		{
			...
			if (typeid(T) == typeid(RenderComponent) && !m_pRenderComponent) m_pRenderComponent = std::make_unique<RenderComponent>(this);
			else if (!HasComponent<T>()) m_pMapComponents[typeid(T)] = std::move(std::make_unique<T>(this, args...));
